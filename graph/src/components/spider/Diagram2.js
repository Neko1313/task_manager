...
const Diagram = () => {
    ...
    if (selectedFile) {
      ...
      try {
        const response = await axios.post(
          'http://localhost:5000/api/scrapy/scrapy_new_spider/des',
          formData,
          {
            headers: {
  ...
  useEffect(() => {
    axios.get('http://localhost:5000/api/scrapy/configuration_des')
      .then(response => {
  ...
  const handlePrintEdges = async () => {
    ...
    if (datas.length > 0){
      try {
        const response = await axios.post(
          'http://localhost:5000/api/scrapy/configuration_des',
          [...new Set(datas)],
          {
            headers: {
